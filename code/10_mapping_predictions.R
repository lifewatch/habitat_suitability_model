...
# INPUT -------------------------------------------------------------------
model_decade <- open_bundle(file.path(datadir, "modelling_decade", "stack_fit.RDS"))
model_month <- open_bundle(file.path(datadir, "modelling_monthly", "stack_fit.RDS"))
...
terra::writeCDF(x = monthly_raster_norm,
                filename = file.path(mapsdir,paste0("HSM_",aphiaid,"_ensemble_","monthly_","v0_1",".nc")),
                varname = "HS",
...
terra::writeCDF(x = pres_decad_raster_norm,
                filename = file.path(mapsdir,paste0("HSM_",aphiaid,"_ensemble_","decade_present_","v0_1",".nc")),
                varname = "HS",
...
for(i in 1:length(future_pred)){
  ...
  terra::writeCDF(x = raster,
                  filename = file.path(mapsdir,paste0("HSM_",aphiaid,"_ensemble_","decade_future_",name,"_v0_1",".nc")),
                  varname = "HS",
